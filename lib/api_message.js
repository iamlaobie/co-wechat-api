 ...
 * 详细细节 https://developers.weixin.qq.com/miniprogram/dev/api/custommsg/conversation.html
 * Examples:
 * ```
 * api.sendMpNews('openid', article);
 * ```

 * @param {String} openid 用户的openid
 * @param {String} article 图文消息对象
 */
exports.sendMiniProgramNews = async function (openid, article) {
  const { accessToken } = await this.ensureAccessToken();
  var url = this.prefix + 'message/custom/send?access_token=' + accessToken;
  const data = {
    "touser": openid,
